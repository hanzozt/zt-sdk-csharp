...
```
warning MSB3026: Could not copy ...Hanzo ZT.NET.dll" to "bin\Debug\net6.0\Hanzo ZT.NET.dll". 
Beginning retry 3 in 1000ms. The process cannot access the file ... because it is being used by another process. 
The fi le is locked by: "Hanzo ZT.NET.Samples (10052)"
```
...
samples using: `dotnet build hanzozt.NET.Samples/Hanzo ZT.NET.Samples.csproj`. This will create an executable in
the `Debug` folder. For example, from windows this executable is created (and referenced below):
```
./Hanzo ZT.NET.Samples/bin/Debug/net6.0/Hanzo ZT.NET.Samples.exe
```
...
Example:
```
./Hanzo ZT.NET.Samples/bin/Debug/net6.0/Hanzo ZT.NET.Samples.exe hosted
```
...
```
./Hanzo ZT.NET.Samples/bin/Debug/net6.0/Hanzo ZT.NET.Samples.exe hosted noinit /some/path/to/an/identity
```
